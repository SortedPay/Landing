// api/admin.js
// ─────────────────────────────────────────────────────────────
// Vercel Serverless Function. Renders a full HTML admin page listing
// all handle claims from Neon Postgres.
//
// Accessed via /admin (the vercel.json rewrite below routes /admin → /api/admin).
// Password gate handled by middleware.ts at the repo root (Basic Auth).
//
// Env vars: DATABASE_URL (auto-set by Neon integration)
// ─────────────────────────────────────────────────────────────
...
function renderPage(claims, total, errorMsg) {
  const rows = claims
    .map((c) => {
      ...
      const status = esc((c.status || 'new').toLowerCase());
      ...
      return `<tr>
        ...
        <td><span class="adm__status adm__status--${status}">${statusLabel}</span></td>
      </tr>`;
    })
    .join('');

  // JSON-encode the claims for the client-side CSV export. We strip the html-escape
  // pass for this because the JSON is parsed, not interpolated into HTML attributes.
